...
from tensorflow.keras.layers import Embedding, Dense, LSTM, Dropout,Conv1D ,GlobalMaxPooling1D
...
### 데이터 불러오기
data =pd.read_csv("https://raw.githubusercontent.com/gyounghwan1313/movie-review-classification/master/movie_200.csv",encoding='euc-kr')
...
#데이터 불러오기
new_test = pd.read_csv("https://raw.githubusercontent.com/gyounghwan1313/movie-review-classification/master/movie_20_test.csv",encoding="euc-kr")
...
data['review']=data['review'].str.replace("[^ㄱ-ㅎㅏ-ㅣ가-힣0-9 ]","")
data['review'].replace('', np.nan, inplace=True)
data['review'].isnull().sum()
data.star=data.star.apply(lambda x: 1 if x>=7 else 0) # 긍정(고평점) / 부정(저평점) 분류

# 단어 전처리 모델
def word_chage_deep(data_set):
    data_set = replace_word('재밋', '재밌', data_set)
    data_set = replace_word('10자\w{0,10}', ' ', data_set)
    data_set = replace_word('ㅜ', 'ㅠ', data_set)
    data_set = replace_word('ㅋ', ' ㅋ ', data_set)
    data_set = replace_word('ㅎ', ' ㅎ ', data_set)
    return data_set
...
x_train=word_chage_deep(x_train) #단어전처리
x_test=word_chage_deep(x_test) #단어전처리

## 토큰화하기
tokenizer = Tokenizer()
tokenizer.fit_on_texts(x_train)
len(tokenizer.word_index) #단어들의 개수 77063
...
for key,value in tokenizer.word_counts.items():
    if value <3:
        rare_word+=1
rare_word #빈도수 2이하인 단어
rare_word/len(tokenizer.word_index)  #0.832를 차지

vocab_size=len(tokenizer.word_index)-rare_word+2 # 전체 단어의 사이즈, 2글자 이하 제거, 패딩 고려
tokenizer = Tokenizer(vocab_size,oov_token="OOV") #  단어 재토큰화
tokenizer.fit_on_texts(x_train)

x_train = tokenizer.texts_to_sequences(x_train)  #단어를 숫자로 변환
x_test = tokenizer.texts_to_sequences(x_test) #단어를 숫자로 변환
...
# 공백인 것은 제거
drop_train=[i for i,w in enumerate(x_train) if len(w) <1]
x_train=np.delete(x_train,drop_train,axis=0)
y_train=np.delete(y_train,drop_train,axis=0)

len(x_train) #25426개의 단어로 학습

## 패딩
max(map(len,x_train)) #리뷰의 최대 길이
sum(map(len,x_train))/len(x_train) #리뷰의 길이 평균
...
mc = ModelCheckpoint('LSTM.h5', monitor='val_acc', mode='max', verbose=1, save_best_only=True)
model.compile(optimizer='adam', loss='binary_crossentropy', metrics=['acc'])

history = model.fit(x_train, y_train, epochs=30, callbacks=[es, mc], batch_size=128, validation_split=0.2)

loaded_model = load_model('LSTM.h5') # bset모델 가져오기

# 정확도
print(loaded_model.evaluate(x_test, y_test)[1]) #0.8348
a=loaded_model.predict(x_test)

y_pred_label=[]
for i in range(len(a)):
    y_pred_label.append(1 if a[i]>.5 else 0)

print(pd.crosstab(y_test,np.array(y_pred_label)))
print(classification_report(y_test,np.array(y_pred_label)))
...
mc = ModelCheckpoint('CNN.h5', monitor = 'val_acc', mode = 'max', verbose = 1, save_best_only = True)
model.compile(optimizer='adam', loss = 'binary_crossentropy', metrics = ['acc'])

history = model.fit(x_train, y_train, epochs = 20, validation_data = (x_test, y_test), callbacks=[es, mc])

loaded_model = load_model('CNN.h5')# bset모델 가져오기

# 정확도
print(loaded_model.evaluate(x_test, y_test)[1]) #0.8495
